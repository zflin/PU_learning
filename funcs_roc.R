...
kappahat <- function(posteriors, Y, n1, n2){
  ...
  denom = rep(0, n_roc)
  for(ii in 1:n_roc){
    numer[ii] = bininv(n2, n2*(1 - tp[ii]), delta)
    denom[ii] = 1 - bininv(n1, n1*fp[ii], delta)
  }
...
cpe <- function(sample1, sample0, train_frac=.5, seed = 2017, ncv=5){
  set.seed(seed)
  ...
  ## make predictions on unpermuted X
  X_orig <- X
  ...
  Z = transformFeatures(X_orig/sigma, Omega, beta)
